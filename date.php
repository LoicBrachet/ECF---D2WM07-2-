...
require ('base.php');
    if(isset($_POST['date'])){
    $date = $_POST['date'];
    echo'<p>Voici les commandes effectuées le '.$date.' : </p>';
    $bdd = new PDO($dsn, $util, $pwd);
        $requete ="SELECT nom, date, quantite, totalHT, totalTTC FROM `client` RIGHT JOIN ventes ON client.idclient = ventes.client_idclient WHERE ventes.date = '".$date."'";
 ...
 ?> 
  <form action="exportdate.php" method="post">
            <select name="date" id="">
            <?php
            require ('base.php');
            $bdd = new PDO($dsn, $util, $pwd);
            $req= "SELECT date FROM ventes WHERE ventes.date = '".$date."'";
            $res= $bdd -> prepare($req);
            $res -> execute();
